...
pub fn sarit_title_search(query: &str, limit: usize, json: bool) -> anyhow::Result<()> {
    ...
    // SARIT は多言語/多表記があるため、GRETIL と同じスコアリングを再利用する。
...
pub fn sarit_fetch(args: &crate::Commands) -> anyhow::Result<()> {
    if let crate::Commands::SaritFetch {
        id,
        query,
        include_notes,
        full,
    ...
    {
        let path = resolve_sarit_path_cli(id.as_deref(), query.as_deref());
...
pub fn sarit_search(
    ...
    if json {
        let meta = serde_json::json!({
            ...
            "results": results,
            "hint": "Use sarit-fetch with the file_id to get full content"
        });
